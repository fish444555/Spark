...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
#     return SparseVector(numSampleOHEFeats, np.sort([OHEDict[x] for x in rawFeats]), [1. for x in range(len(rawFeats))])
    temp = np.sort([OHEDict[x] for x in rawFeats if OHEDict.has_key(x)])
    return SparseVector(numOHEFeats, temp, np.ones(len(temp)) * 1.)

#     return SparseVector(numSampleOHEFeats, np.sort([OHEDict[x] for x in rawFeats]), np.ones(len(rawFeats)) * 1.)
...
def parsePoint(point):
    ...
    """
    return [ i for i in enumerate(point.split(',')[1:])]
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    temp = np.sort([OHEDict[x] for x in rawFeats if OHEDict.has_key(x)])
    return SparseVector(numOHEFeats, temp, np.ones(len(temp)) * 1.)
#     return SparseVector(numOHEFeats, np.sort([OHEDict[x] for x in rawFeats if OHEDict.has_key(x)]), np.ones(len(rawFeats)) * 1.)
#     return SparseVector(numOHEFeats, np.sort([OHEDict[x] for x in rawFeats if OHEDict.has_key(x)]), [1. for x in range(len(rawFeats))])
...
def computeLogLoss(p, y):
    ...
    if y == 1:        
        ...
        res = -log( 1 - p)
#     return y == 0? -log(p + epsilon): -log( 1 - p + epsilon)
    return res
...
#     return model.weights.dot(data.features)
...
def computeSparsity(data, d, n):
    ...
    """
    return data.map(lambda p: len(p.features.indices)).sum() * 1. / (d * n)
