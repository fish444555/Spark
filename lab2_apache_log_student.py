...
# endpointCountPairTuple = not200.reduceByKey(lambda a, b: a + b)
...
print 'Found %d response codes' % len(responseCodeToCount.collect())
...
# hosts = access_logs.map(lambda log: log.hosts)
hosts = (access_logs
       .map(lambda log: (log.host, 1))
       .reduceByKey(lambda a, b : a + b)
       .cache())

uniqueHostCount = len(hosts.collect())
...
dayGroupedHosts = (access_logs
                  .map(lambda log: (log.date_time.day, log.host))
                  .groupByKey())

dayHostCount = len(dayGroupedHosts.collect())

dailyHosts = dayGroupedHosts.map(lambda (k, v): (k, len(set(v)))).sortByKey().cache()
...
dailyHosts.map(lambda (k, v): k).collect()
...
avgDailyReqPerHost = (sortedByDay
                      .map(lambda (k, v): (k, len(v)/len(set(v))))
                      .cache())
...
badEndpointsSum = badRecords.map(lambda log: (log.endpoint, 1)).reduceByKey(lambda a, b: a+b)

badEndpointsTop20 = badEndpointsSum.sortBy(lambda x: x[1], ascending=False).collect()[:20]
...
errHostsSum = badRecords.map(lambda log: (log.host, 1)).reduceByKey(lambda a, b: a+b)

errHostsTop25 = errHostsSum.sortBy(lambda x: x[1], ascending=False).collect()[:25]
...
# errDateCountPairTuple = badRecords.map(lambda log: log.date_time.day).reduceByKey(lambda a, b: a+b)

errDateSum = badRecords.map(lambda log: (log.date_time.day, 1)).reduceByKey(lambda a, b: a+b)
...
topErrDate = errDateSorted.sortBy(lambda x: x[1], ascending=False).collect()[:5]
...
# hourCountPairTuple = badRecords.map(lambda log: (log.date_time.hour, 1)).reduceByKey(lambda a, b: a+b)

hourRecordsSum = badRecords.map(lambda log: (log.date_time.hour, 1)).reduceByKey(lambda a, b: a+b)
