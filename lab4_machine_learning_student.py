...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    return IDandRatingsTuple[0], (len(IDandRatingsTuple[1]), sum(IDandRatingsTuple[1])/float(len(IDandRatingsTuple[1])))
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(lambda (x, y): getCountsAndAverages((x,y)))
...
movieNameWithAvgRatingsRDD = (movieIDsWithAvgRatingsRDD
                              .join(moviesRDD)
                              .map(lambda (x, (y, z)): (y[1], z, y[0])))
...
def computeError(predictedRDD, actualRDD):
    ...
    totalError = (squaredErrorsRDD
                 .map(lambda (x, y): (float(x) - float(y)) * (float(x) - float(y)))
                 .sum())
#     totalError = t1.map(lambda (x, y): (float(x)-float(y))*(float(x)-float(y)))
# totalError.take(3)
...
trainingAvgRating = trainingRDD.map(lambda (x, y, z): z).sum() / trainingRDD.count()
...
myUnratedMoviesRDD = (moviesRDD
                      ...
                      .subtract(sc.parallelize(myRatedMovies)
                                .map(lambda (x, y, z): (x, y))))

# t1 = moviesRDD.map(lambda (x, y): (myUserID, x) )
# t2 = t1.subtract(sc.parallelize(myRatedMovies).map(lambda (x, y, z): (x, y)))
