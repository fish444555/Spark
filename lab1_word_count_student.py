...
def wordCount(wordListRDD):
    ...
    """
    return wordListRDD.map(lambda k: (k, 1)).groupByKey().map(lambda (k, v): (k, sum(v)))
...
# TODO: Replace <FILL IN> with appropriate code
shakespeareWordsRDD = shakespeareRDD.flatMap(lambda x: x.split(" "))#.filter(lambda x: len(x)>0)
...
# TODO: Replace <FILL IN> with appropriate code
top15WordsAndCounts = sorted(wordCount(shakeWordsRDD).collect(), cmp=lambda x,y:cmp(x[1],y[1]), reverse = True)[:15]
