...
def parsePoint(line):
    ...
    """
    res = line.split(',')
    return LabeledPoint(float(res[0]), [float(x) for x in res[1:]])
...
parsedTrainData.map(parsePoint, parsedTrainData)
parsedValData.map(parsePoint, parsedValData)
parsedTestData.map(parsePoint, parsedTestData)
...
def calcRMSE(labelsAndPreds):
    ...
    """
    import math
    return math.sqrt(labelsAndPreds.map(lambda (x, y): squaredError(x, y)).sum() / labelsAndPreds.count())
...
# TODO: Replace <FILL IN> with appropriate code
labelsAndPredsTrain = parsedTrainData.map(lambda x: x.label).map(lambda x: (x, averageTrainYear))
rmseTrainBase = calcRMSE(labelsAndPredsTrain)

labelsAndPredsVal = parsedValData.map(lambda x: x.label).map(lambda x: (x, averageTrainYear))
rmseValBase = calcRMSE(labelsAndPredsVal)

labelsAndPredsTest = parsedTestData.map(lambda x: x.label).map(lambda x: (x, averageTrainYear))
...
labelsAndPreds = parsedValData.map(lambda x: x.label).map(lambda x: (x, averageTrainYear))
...
firstModel = LinearRegressionWithSGD.train(parsedTrainData, iterations=numIters, miniBatchFraction=miniBatchFrac
...
def twoWayInteractions(lp):
    ...
    """
    newf = list(itertools.product(lp.features, lp.features))
    newf2 = [x*y for (x, y) in newf]
    return LabeledPoint(lp.label, np.hstack((lp.features, newf2)))
