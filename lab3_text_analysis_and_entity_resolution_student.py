...
def simpleTokenize(string):
    ...
    """
    temp = re.split(split_regex, string.lower())
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda rec: (rec[0], tokenize(rec[1]))).reduceByKey(lambda a, b: a + b)
googleRecToToken = googleSmall.map(lambda rec: (rec[0], tokenize(rec[1]))).reduceByKey(lambda a, b: a + b)
...
def countTokens(vendorRDD):
    ...
    """
    return sum(vendorRDD.map(lambda (k, v): len(v)).collect())
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.sortBy(lambda x: len(x[1]), ascending=False).collect()
...
def tf(tokens):
    ...
    for i in tokens:
        if t_list.has_key(i):
            ...
            t_list[i] = 1
    count = len(tokens)
    for x in t_list.iteritems():
        t_list[x[0]] = x[1] / float(count)
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMap(lambda (k, v): v).distinct()
    t2 = corpus.flatMap(lambda (k, v): list(set(v)))#.reduce(lambda a, b: a+b) #.collect()
    t3 = t2.collect()
    tokenSumPairTuple = uniqueTokens.map(lambda t: (t, N/float(t3.count(t)) ))
...
def tfidf(tokens, idfs):
    ...
    for i in tokens:
        if idfs.has_key(i):
            tfIdfDict[i] = tokens.count(i) / float(N) * idfs[i]
        else:
            tfIdfDict[i] = tokens.count(i) / float(N) 
    return tfIdfDict
...
avgSimNon = sum(nonDupsRDD.map(lambda x: x[1]).collect())/ nonDupsRDD.count()
...
# TODO: Replace <FILL IN> with appropriate code
amazonFullRecToToken = amazon.map(lambda rec: (rec[0], tokenize(rec[1]))).reduceByKey(lambda a, b: a + b)
googleFullRecToToken = google.map(lambda rec: (rec[0], tokenize(rec[1]))).reduceByKey(lambda a, b: a + b)
...
# TODO: Replace <FILL IN> with appropriate code
fullCorpusRDD = googleFullRecToToken.union(amazonFullRecToToken)
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda (k, v): (k, tfidf(v, idfsFullWeights)))
googleWeightsRDD = googleFullRecToToken.map(lambda (k, v): (k, tfidf(v, idfsFullWeights)))
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.collectAsMap()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.collectAsMap()
...
def invert(record):
    ...
    return [[record[1].keys()[0], record[0]]]
# amazonWeightsRDD.flatMap(lambda (k,v): {x: k for x in v}.items())

amazonInvPairsRDD = (amazonWeightsRDD
                    .flatMap(lambda (k, v): {x: k for x in v}
                    .items())
                    .cache())
...
def fastCosineSimilarity(record):
    ...
    sum0 = 0.
    for i in tokens:
        sum0 += amazonWeightsBroadcast.value[amazonRec][i] * googleWeightsBroadcast.value[googleRec][i]
    value = sum0 / (norm(googleWeightsBroadcast.value[URL]) * norm(amazonWeightsBroadcast.value[ID]))
    key = (amazonRec, googleRec)
    return (key, value)

similaritiesFullRDD = (commonTokens
                       .map(lambda ((x, y), z): fastCosineSimilarity(((x, y), z)))
                       .cache())
